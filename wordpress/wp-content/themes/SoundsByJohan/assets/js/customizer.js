		...
		_init: function(){
			...
			control.actions = function( $context ){
				if ( control.params.live_title_id ) {
					...
					if ( control.id === 'SoundsByJohan_section_order_styling' ) {
...
jQuery( window ).ready( function( $ ){
    if ( typeof SoundsByJohan_customizer_settings !== "undefined" ) {
        if (SoundsByJohan_customizer_settings.number_action > 0) {
            $('.control-section-themes h3.accordion-section-title').append('<a class="theme-action-count" href="' + SoundsByJohan_customizer_settings.action_url + '">' + SoundsByJohan_customizer_settings.number_action + '</a>');
        }
/*        if ( SoundsByJohan_customizer_settings.is_plus_activated !== 'y' ) {
            $('#customize-info .accordion-section-title').append('<a target="_blank" style="text-transform: uppercase; background: #D54E21; color: #fff; font-size: 10px; line-height: 14px; padding: 2px 5px; display: inline-block;" href="https://www.famethemes.com/themes/SoundsByJohan/?utm_source=theme_customizer&utm_medium=text_link&utm_campaign=SoundsByJohan_customizer#get-started">Upgrade to SoundsByJohan plus</a>');
            $( '#accordion-section-SoundsByJohan_order_styling > .accordion-section-title').append( '<span class="SoundsByJohan-notice">Plus</span>' );
        }*/
    }
	 ...
	 */
	$( 'select[data-customize-setting-link="SoundsByJohan_hero_layout"]').on( 'change on_custom_load', function(){
		var v = $( this).val() || '';

		$( "li[id^='customize-control-SoundsByJohan_hcl']").hide();
		$( "li[id^='customize-control-SoundsByJohan_hcl"+v+"']").show();

	} );

	$( 'select[data-customize-setting-link="SoundsByJohan_hero_layout"]').trigger( 'on_custom_load' );
