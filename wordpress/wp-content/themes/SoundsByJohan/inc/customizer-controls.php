...
/*-----------------------------------------------------------------------------------*/
/*  SoundsByJohan Customizer Controls
/*-----------------------------------------------------------------------------------*/

class SoundsByJohan_Misc_Control extends WP_Customize_Control {
...
class SoundsByJohan_Theme_Support extends WP_Customize_Control {
	public function render_content() {
		echo wp_kses_post( 'Upgrade to <a href="#">SoundsByJohan Plus</a> to be able to change the section order and styling!', 'SoundsByJohan' );
	}
}

if ( ! function_exists( 'SoundsByJohan_sanitize_checkbox' ) ) {
...
function SoundsByJohan_sanitize_color_alpha( $color ){
 ...
 */
function SoundsByJohan_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = SoundsByJohan_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  SoundsByJohan_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
class SoundsByJohan_Editor_Custom_Control extends WP_Customize_Control
{
    /**
    ...
    public function enqueue() {
        ...
        wp_register_script( 'SoundsByJohan-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-controls', 'wp-color-picker' ) );
        wp_register_style( 'SoundsByJohan-customizer',  get_template_directory_uri() . '/assets/css/customizer.css' );

        wp_enqueue_script( 'SoundsByJohan-customizer' );
        wp_enqueue_style( 'SoundsByJohan-customizer' );
 ...
 */
class SoundsByJohan_Alpha_Color_Control extends WP_Customize_Control {
 ...
 * @access public
 */
class SoundsByJohan_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'SoundsByJohan_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
            $this->max_item = absint( $args['max_item'] );
        }

        $this->changeable =  isset(  $args['changeable'] ) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset(  $args['default_empty_title'] ) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__( 'Item', 'SoundsByJohan' );
    ...
    public function render_content() {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php _e( 'Add an item', 'SoundsByJohan' ); ?></span>
        </div>
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php _e( 'Item', 'SoundsByJohan' ); ?></h4>
                    </div>
                </div>
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    #>
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php _e( 'Remove', 'SoundsByJohan' ) ?></button>
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'SoundsByJohan' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'SoundsByJohan' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add', 'SoundsByJohan' ); ?> <# } else { #> <?php _e( 'Change', 'SoundsByJohan' ); ?> <# } #> </button>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php _e( 'Remove', 'SoundsByJohan' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php _e( 'Close', 'SoundsByJohan' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
