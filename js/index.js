...
function editImage(s, i) {
			...
			$("." + imageToBeEdited + "").css("filter", 'grayscale(' + gs + '%) blur(' + blur + 'px) brightness(' + br + '%) contrast(' + ct + '%) hue-rotate(' + huer + 'deg) opacity(' + opacity + '%) invert(' + invert + '%) saturate(' + saturate + '%)' //sepia(' + sepia + '%)'
			);

			$("." + imageToBeEdited + "").css("-webkit-filter", 'grayscale(' + gs + '%) blur(' + blur + 'px) brightness(' + br + '%) contrast(' + ct + '%) hue-rotate(' + huer + 'deg) opacity(' + opacity + '%) invert(' + invert + '%) saturate(' + saturate + '%)' // sepia(' + sepia + '%)'
			);
}
...
function destroyImage(t) {
			if (confirm("Are you sure?")) {
						if (TogetherJS.running) {
									...
									var location = elementFinder.elementLocation(t);

									TogetherJS.send({ type: "destroyImage", destroyI: location });
						}
...
function addImage(e) {
			...
			if (imgUrl.length) {
						...
						removeButton.setAttribute("onclick", "destroyImage(this)");
